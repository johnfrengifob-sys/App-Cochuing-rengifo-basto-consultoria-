...
export const ClientDirectoryTable: React.FC<ClientDirectoryTableProps> = ({
  ...
  const handleManualCloudSync = async () => {
    ...
    try {
      const { usersCount, regsCount } = await FirestoreSyncService.syncAllFromFirestore();
      if (onRefreshClients) {
        onRefreshClients();
      }
      setActionToast({
        message: `Sincronización en la nube exitosa: ${usersCount} usuarios y ${regsCount} registros actualizados.`,
        type: 'success',
      });
    } catch {
      setActionToast({
        message: 'No se pudo sincronizar con Firestore en este momento.',
        type: 'error',
      });
    } finally {
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="glass-panel-opal rounded-3xl p-5 sm:p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Action Tools: Sync, New Client Button & Table vs Cards Toggle */}
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              id="btn-sync-cloud-clients"
              disabled={isSyncingCloud}
              onClick={handleManualCloudSync}
              className="px-3 py-2 rounded-xl bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-200 border border-gray-200 dark:border-neutral-700 hover:bg-gray-50 dark:hover:bg-neutral-700 text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs disabled:opacity-50"
              title="Sincronizar directorio con base de datos en la nube (Firestore)"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isSyncingCloud ? 'animate-spin text-emerald-500' : ''}`} />
              <span className="hidden sm:inline">Sincronizar Cloud</span>
            </button>
