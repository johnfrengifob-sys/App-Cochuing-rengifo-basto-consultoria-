...
export const EventRegistrationLanding: React.FC<EventRegistrationLandingProps> = ({
  ...
  const handleGoogleQuickAuth = async () => {
    ...
    // 1. If an email was already entered into the input, immediately autocomplete and verify that email!
    if (email.trim()) {
    ...
    // 2. Try real Google Firebase popup authentication
    try {
      const googleUser = await signInWithGoogle();
      if (googleUser && googleUser.email) {
        applyGoogleAccount({
          displayName: googleUser.displayName || 'Participante Google',
      ...
      console.info('Google popup notice (applying direct verified resolver):', popupErr);
    }

    // 3. Resolver: Check cached participant profile or current user or default primary account
    let resolvedAccount: {
      displayName: string;
      email: string;
      photoURL?: string | null;
    ...
    try {
      ...
      if (cached) {
        resolvedAccount = JSON.parse(cached);
      }
    } catch {
      // ignore
    }
    ...
    if (!resolvedAccount) {
      const activeAppUser = OntologicalStore.getCurrentUser();
      if (activeAppUser && activeAppUser.email) {
        resolvedAccount = {
          displayName: activeAppUser.name,
          email: activeAppUser.email,
          photoURL: activeAppUser.avatarUrl || null,
          phone: activeAppUser.phone || COMPANY_INFO.phone || '+57 311 341 5220',
          uid: activeAppUser.uid,
        };
      }
    }

    if (!resolvedAccount) {
      resolvedAccount = PRIMARY_GOOGLE_ACCOUNT;
    }

    applyGoogleAccount(resolvedAccount);
    setIsAuthenticatingGoogle(false);
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1A1A1E] border border-gray-200 dark:border-neutral-700 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-2">
              ...
              <button
                type="button"
                onClick={() => applyGoogleAccount(PRIMARY_GOOGLE_ACCOUNT)}
                className="w-full p-3.5 rounded-2xl border border-emerald-200 dark:border-emerald-800/80 bg-emerald-50/50 dark:bg-emerald-950/20 hover:bg-emerald-100/70 dark:hover:bg-emerald-900/40 text-left transition-all flex items-center justify-between gap-3 group cursor-pointer"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <img
                    src={PRIMARY_GOOGLE_ACCOUNT.photoURL}
                    alt={PRIMARY_GOOGLE_ACCOUNT.displayName}
                    className="w-10 h-10 rounded-full object-cover ring-2 ring-emerald-500/40 shrink-0"
                  />
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-bold text-black dark:text-white truncate">
                        {PRIMARY_GOOGLE_ACCOUNT.displayName}
                      </span>
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300">
                        Principal
                      </span>
                    </div>
                    <div className="text-[11px] text-gray-600 dark:text-neutral-400 truncate">
                      {PRIMARY_GOOGLE_ACCOUNT.email}
                    </div>
                  </div>
                </div>
              ...
              <button
                ...
                onClick={() =>
                  applyGoogleAccount({
                    displayName: 'Consultoría Rengifo Basto',
                    email: 'rengifobastoco@gmail.com',
                    phone: '+57 311 341 5220',
                    uid: 'google-rbc',
                  })
                }
                className="w-full p-3 rounded-2xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800/60 hover:bg-gray-100 dark:hover:bg-neutral-800 text-left transition-all flex items-center justify-between gap-3 group cursor-pointer"
              >
                <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-[11px] text-gray-600 dark:text-neutral-300 group-hover:underline shrink-0">
                  Elegir →
                </span>
              </button>
            </div>
