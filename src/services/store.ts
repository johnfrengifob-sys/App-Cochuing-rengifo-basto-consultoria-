...
export let PROGRAM_NODES: ProgramNodeInfo[] = [...OFFICIAL_PROGRAM_NODES];
...
const INITIAL_SESSIONS: Session[] = generateOfficialSessions('client-legadobarber2026');
...
export class OntologicalStore {
  ...
  static convertProspectToClient(
    ...
    const firstSession: Session = {
      ...
      status: 'scheduled',
      notes: 'Sesión 1: Espacio de Indagación Autónoma y Construcción de Sentido.',
    };
  ...
  static createManualClient(data: {
    ...
    const firstSession: Session = {
      ...
      meetLink: `https://meet.google.com/rbc-${Math.random().toString(36).substring(2, 7)}`,
      status: 'scheduled',
      notes: `Sesión 1: Espacio de Indagación Autónoma y Construcción de Sentido para ${newClient.name}.`,
      durationMinutes: 60,
    };
    this.save(STORAGE_KEYS.SESSIONS, [...sessions, firstSession]);
  ...
  static getSessions(): Session[] {
    ...
    if (safeList.length === 0) {
      safeList = generateOfficialSessions('client-legadobarber2026');
    }

    const levelConfigs = this.getLevelConfigs();
    const programNodes = this.getProgramNodes();
    const nodeMap = new Map<number, ProgramNodeInfo>();
    programNodes.forEach((n) => nodeMap.set(n.step, n));

    safeList.forEach((s, idx) => {
      ...
      if (!title || title.includes('1- Mapeo') || title.includes('Espacio de Exploración') || title.includes('Sesión #') || title.startsWith(`Sesión ${step}: ${step}-`)) {
        title = node ? `Sesión ${step}: ${node.sessionTitle}` : `Sesión ${step}: ${isCierre ? 'Cierre de Ciclo: Integración, Cosecha de Aprendizajes y Evolución del Ser' : 'Espacio de Indagación Autónoma y Construcción de Sentido'}`;
      }
      s.title = title;

      if (!s.calendarLink || s.calendarLink.includes('undefined')) {
        s.calendarLink = DEFAULT_CALENDAR_URL;
      }
      if (!s.meetLink) {
        s.meetLink = 'https://meet.google.com/rbc-conversatorio-ontologico';
      }
      if (!s.sessionGoal || s.sessionGoal.includes('Acompañamiento ontológico no direccional')) {
        s.sessionGoal = node?.objective || (isCierre
          ? 'Acompañar al cliente en la integración reflexiva del proceso recorrido, facilitando un espacio de autoconocimiento donde reconozca sus propias transformaciones, consolide los aprendizajes clave derivados de su experiencia y proyecte con autonomía sus siguientes pasos y compromisos de desarrollo.'
          : 'Facilitar un espacio de reflexión profunda donde el cliente explore su propia realidad, identifique nuevas distinciones y potencie su aprendizaje autónomo.');
      }
      if (!s.openingQuestion || s.openingQuestion.includes('¿Qué es importante para ti traer')) {
        s.openingQuestion = node?.keyQuestion || (isCierre
      ...
      if (!s.notes || s.notes.includes('Pregunta de apertura') || !s.notes.includes('Requisitos:')) {
        s.notes = node?.levelPrompt ? `Requisitos: ${node.levelPrompt}` : (isCierre
  ...
  static generateDefaultSessionsForClient(clientId: string): Session[] {
    ...
    for (let num = 1; num <= sessionCount; num++) {
      ...
      sessions.push({
        ...
        isPaid: true,
        durationMinutes: 60,
        sessionGoal: nodeInfo?.objective || (isMilestone
          ? 'Acompañar al cliente en la integración reflexiva del proceso recorrido, facilitando un espacio de autoconocimiento donde reconozca sus propias transformaciones, consolide los aprendizajes clave derivados de su experiencia y proyecte con autonomía sus siguientes pasos y compromisos de desarrollo.'
          : 'Facilitar un espacio de reflexión profunda donde el cliente explore su propia realidad, identifique nuevas distinciones y potencie su aprendizaje autónomo.'),
        openingQuestion: nodeInfo?.keyQuestion || (isMilestone
          ? '¿Qué nuevo observador emerge en ti al reconocer tu evolución y qué compromisos eliges proyectar?'
          : '¿Qué aspecto de tu realidad requiere hoy una indagación profunda para construir un sentido renovado?'),
        ontologicalFocus: nodeInfo?.sessionTitle || (isMilestone ? 'Cierre de Ciclo: Integración, Cosecha de Aprendizajes y Evolución del Ser' : 'Espacio de Indagación Autónoma y Construcción de Sentido'),
        notes: nodeInfo?.levelPrompt
          ? `Requisitos: ${nodeInfo.levelPrompt}`
          : (isMilestone
            ? 'Requisitos: Disposición para la autoobservación profunda, apertura para reconocer los logros y quiebres superados durante el proceso, y un nivel de presencia plena para evaluar el impacto de su propia evolución sin expectativas externas.'
            : 'Requisitos: Disposición para la autoobservación, apertura a la incertidumbre y un entorno seguro y libre de juicios.'),
        programNodeStep: num,
...
Rengifo Basto Consultoría Ontológica`;
  ...
  // AISLAMIENTO ESTRICTO DE PRIVACIDAD: Obtener solo las bitácoras del participante activo
  public static getBitacorasSesionesB2BForClient(clientEmailOrUid: string): BitacoraSesionB2BEntry[] {
    const query = (clientEmailOrUid || '').toLowerCase().trim();
    if (!query) return [];
    const all = this.getBitacorasSesionesB2B();
    return all.filter((b) => {
  ...
  // AISLAMIENTO ESTRICTO DE PRIVACIDAD: Obtener solo los acuerdos del participante activo
  public static getSesionIndividualAcuerdosForClient(clientEmailOrUid: string): SesionIndividualAcuerdoEntry[] {
    const query = (clientEmailOrUid || '').toLowerCase().trim();
    if (!query) return [];
    const all = this.getSesionIndividualAcuerdos();
    return all.filter((a) => {
  ...
  // AISLAMIENTO ESTRICTO DE PRIVACIDAD: Obtener solo las bitácoras de talleres del participante activo
  public static getBitacorasTalleresForClient(clientEmailOrUid: string): BitacoraTallerEntry[] {
    const query = (clientEmailOrUid || '').toLowerCase().trim();
    if (!query) return [];
    const all = this.getBitacorasTalleres();
    return all.filter((b) => {
      const e = (b.email || '').toLowerCase().trim();
      return e === query;
    });
  ...
  // SINCRONIZACIÓN OFICIAL EN TIEMPO REAL CON GOOGLE SHEETS
  // Hoja oficial vinculada: 1Mm3CRZVvKYFak5APwIBmfK-vZAUfnx1zg-eq8WOLbZk
  public static async syncBitacorasFromGoogleSheets(): Promise<{
    success: boolean;
    totalRows: number;
    newCount: number;
    totalStored: number;
    lastSyncedAt: string;
    items: BitacoraSesionB2BEntry[];
  }> {
    try {
      const res = await fetch('/api/integrations/sync-bitacoras-sheets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (res.ok) {
        ...
        if (Array.isArray(data.items) && data.items.length > 0) {
          this.save('rbc_bitacoras_sesiones_b2b', data.items);
          this.updateFormsSheetsCount('bitacora_sesiones_b2b', data.items.length);
          this.updateFormsSheetsIntegration('bitacora_sesiones_b2b', {
            status: 'connected',
            lastSyncedAt: data.lastSyncedAt || new Date().toISOString(),
            recordsCount: data.items.length,
          });
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('rbc-forms-sheets-data-updated'));
            window.dispatchEvent(new CustomEvent('rbc-bitacoras-sheets-synced', { detail: data }));
          }
          return data;
        }
      }
    } catch (err) {
      console.warn('syncBitacorasFromGoogleSheets notice:', err);
    }
  ...
  public static getUnifiedClientOntologicalCrossData(
    ...
    const primaryIsEmail = isEmail(query);
    ...
    const matchesQuery = (emailStr?: string, nameStr?: string) => {
      ...
      // Si se pasa un correo electrónico, la coincidencia de aislamiento por correo debe ser estricta
      if (primaryIsEmail && e) {
        return e === query;
      }
      if (query && (e === query || (n && n === query))) return true;
      if (query2 && isEmail(query2) && e) return e === query2;
      if (query2 && (e === query2 || (n && n === query2))) return true;
